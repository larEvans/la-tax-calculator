...
# ── PYINSTALLER BUNDLE / FROZEN CHECK ───────────────────────────────────────────
...
class Entry(db.Model):
    ...
    # <-- cascade so that Income rows go away with the Entry
...
# Now that Entry is defined, create the table if it doesn’t exist
...
def show_taxes():
    ...
    for i in range(n):
        ...
        # append a full row, including Date
        records.append([
            sender,
            inc_type,
            check_date,
...
def show_taxes():
    ...
    # Constants for self-employment tax
    ...
    for i in range(n):
        ...
        if inc_type == '1099-NEC':
            ...
            # Social Security only up to the wage base
...
def expense_entry():
    ...
    # Build a dict of saved inputs from the CSV
    saved = {}
    # Initialize counters for each check
    counts = {i: 0 for i in range(num_checks)}
    if exp_csv:
        df_exp = pd.read_csv(StringIO(exp_csv))
        # ['Sender','Expense','Amount','Net Profit']
        for _, row in df_exp.iterrows():
            sender = row['Sender']
            # find which check index this sender corresponds to
    ...
    # Ensure at least one row per check
    for i in range(num_checks):
...
def show_final():
    ...
    exp_data = {}
    # We need df_tax to know how many checks
    df_tax = pd.read_csv(StringIO(request.form['tax_csv']))
    for i, sender in enumerate(df_tax['Sender']):
        # how many expense rows for this check
...
def save_entry():
    ...
    # now parse out each row of the tax CSV
...
def delete_entry(entry_id):
    # remove all incomes linked to this entry
...
def view_entry(entry_id):
    ...
    # --- compute all your summary numbers ---
    comp_labels = ['Self-EE','Fed','State']
    comp_data = [
        df_tax['Self-EE Tax'].sum(),
        df_tax['Fed Tax'].sum(),
...
def download_final():
    ...
    # each row: we assume df_exp already has columns ['Sender','Name','Amount','Net Profit']
...
def statements():
    ...
    # ensure required columns even if empty
    ...
    # 3) pull all expenses, mapping each to its check date
    exp_rows = []
    for entry in Entry.query.all():
        if not entry.exp_csv.strip():
            continue
        df_exp = pd.read_csv(StringIO(entry.exp_csv))
        # make a sender→date map for this entry
...
def download_entry(entry_id):
    ...
    # embed the same two pies you show in the UI
...
def download_statements():
    ...
    # 1) Load all Income records (optionally filter by type)
    q = Income.query
    if filter_type != 'All':
        q = q.filter_by(income_type=filter_type)
    incs = q.order_by(Income.date).all()

    # 2) Build DataFrame of incomes
    inc_df = pd.DataFrame([{
        'date':         i.date,
        'Gross':        i.Gross,
    ...
    # 3) Load all Expenses (tie back to entry & sender)
    #    We’ll store: entry_date, sender, name, amt
    exp_rows = []
    for e in Entry.query.all():
        # parse each saved exp_csv
        try:
    ...
    # 4) Compute monthly & yearly aggregates
    inc_df['month'] = inc_df['date'].dt.to_period('M').dt.to_timestamp()
    inc_df['year']  = inc_df['date'].dt.year
...
def show_final():
    ...
    comp_data = [
      ...
      # Build list of non‐empty expense lines
    exp_lines = []
    total_exp  = 0.0
    ...
    for i, sender in enumerate(df_tax['Sender']):
        ...
        for j in range(cnt):
            name    = request.form.get(f'exp_name_{i}_{j}', '').strip()
            amt_str = request.form.get(f'exp_amt_{i}_{j}', '').strip()
            # skip any row where name or amount is blank
            if not name or not amt_str:
                continue
            amt = float(amt_str)
            exp_lines.append((sender, name, amt))
